...
+ [新增] 画布提示词框支持点击、拖入或粘贴参考图，按实际发送顺序显示图1、图2并随图片请求一同提交。
+ [调整] 4K 主供应商和备用供应商支持分别填写模型名称，请求时按各自配置调用。

## v0.15.5 - 2026-08-11

+ [修复] Windows 超分启动文件改用稳定的命令编码，避免双击后窗口闪退。

## v0.15.4 - 2026-08-11

+ [修复] 本机 Upscayl 服务改用独立端口，仅允许已发布画布和本地开发页面访问，并兼容浏览器的本地网络访问检查。
+ [修复] Upscayl 启动程序在未安装系统 Node.js 时自动使用本机 Codex 附带的 Node.js。

## v0.15.3 - 2026-08-11

+ [修复] 将旧浏览器配置和已有画布节点的默认生图数量迁移为 1 张。
+ [调整] 生成配置只保留 `gpt-image-2` 生图，移除文本生成、视频和音频模式；手动文字节点继续保留。
+ [调整] 默认渠道只保留 `gpt-image-2`，偏好设置隐藏视频和音频选项。

## v0.15.2 - 2026-08-11

+ [新增] 图像设置支持配置 4K 主供应商和备用供应商，主供应商请求失败时自动切换备用供应商，模型统一固定为 `gpt-image-2`。
+ [新增] AI 返图后自动调用本机 Upscayl 按固定参数放大 2 倍；局部修改会先按蒙版合成完整海报再超分，超分不可用时保留 AI 原图并提示。
+ [修复] 局部编辑在 OpenAI 兼容中转下强制使用 `/images/edits`，确保旧的自定义脚本不会丢失蒙版。
+ [修复] 局部结果按透明蒙版贴回原海报，选区外继续保留原图像素、日期和文字。
+ [调整] 画布默认生图张数改为 1，并将旧版默认值迁移为 1。
+ [修复] 矩形和套索工具显示可见的十字光标。

+ [新增] 图像设置和画布尺寸选择器加入 9 种易拉宝尺寸：1712x3840、1440x3840、1504x3840、1888x3840、2208x3680、2336x3504、2400x3360、1728x3840、1920x3840。
+ [新增] 局部图片编辑支持画笔、矩形和套索选区，并在 AI 返回后仅合成选区内容，选区外保留原图像素。
+ [修复] 局部编辑会自动放大选区后再交给 AI，并以高保真模式贴回原位置，提升易拉宝小文字替换的成功率。
+ [调整] 画布中的海报图片改为直角显示，避免圆角裁掉画面内容。
+ [调整] 默认 OpenAI 兼容接口改为 `https://api.qiuqiutoken.com/v1`，模型保持 `gpt-image-2`。
+ [调整] GitHub Pages 工作流改为推送 `main` 分支后自动构建发布。
+ [优化] 网站首页直接进入最近画布，首次使用且本浏览器没有 API Key 时自动打开简化设置。
